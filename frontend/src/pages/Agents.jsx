...
export default function Agents() {
  ...
  const handleDelete = async (agent) => {
    if (!confirm(t('agents.confirmDelete', { name: agent.agent_name }))) return
    try { await deleteAgent(agent.id); load() }
    catch (err) { alert(err.response?.data?.detail || t('agents.genericError')) }
  }
  ...
  const handleDuplicate = (agent) => {
    ...
    setModal({ ...rest, agent_name: `${t('agents.copyOf')}${agent.agent_name}`, name: `${t('agents.copyOf')}${agent.name}` })
  }
  ...
  const handleSync = async (agent) => {
    ...
    try {
      ...
      if (resp.retell_error) {
        alert(t('agents.syncErrorPrefix') + resp.retell_error)
      }
      load()
    } catch (err) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {agents.map(agent => (
          <div key={agent.id} className="bg-z-card rounded-xl p-5 border border-z-border">
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {agent.is_default && (
                    <span className="px-1.5 py-0.5 bg-yellow-500/15 text-yellow-400 text-xs font-semibold rounded-full">{t('agents.default')}</span>
                  )}
                  {agent.retell_agent_id ? (
                    ...
                    <span className="flex items-center gap-1 px-1.5 py-0.5 bg-amber-500/15 text-amber-400 text-xs font-semibold rounded-full">
                      <ExclamationTriangleIcon className="w-3 h-3" /> {t('agents.notSynced')}
                    </span>
                  )}
                  {agent.inbound_enabled && (
                    <span className="flex items-center gap-1 px-1.5 py-0.5 bg-z-blue/15 text-z-blue-light text-xs font-semibold rounded-full">
                      <PhoneArrowDownLeftIcon className="w-3 h-3" /> {t('agents.inboundActive')}
                    </span>
                  )}
                </div>
            ...
            <p className="text-xs text-slate-500 mb-4">
              ...
              {' • '}{t('agents.maxDuration', { s: agent.max_call_duration })}
            </p>
            ...
            <div className="flex gap-2 flex-wrap">
              ...
              <button
                ...
                title={agent.retell_agent_id ? t('agents.testTooltip') : t('agents.testTooltipLocked')}
              ...
              >
                <PhoneIcon className="w-3.5 h-3.5" /> {t('agents.test')}
              </button>
        ...
        {agents.length === 0 && (
          <div className="col-span-full text-center py-16">
            ...
            <p className="text-slate-500 text-sm">{t('agents.noAgentsHint')}</p>
          </div>
        )}
      </div>
