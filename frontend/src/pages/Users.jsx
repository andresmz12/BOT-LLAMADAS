...
export default function Users() {
  ...
  const handleDelete = async (member) => {
    if (!confirm(t('team.confirmDelete', { name: member.full_name }))) return
    try { await deleteTeamMember(member.id); load() }
    catch (err) { alert(err.response?.data?.detail || t('team.errorDelete')) }
  }
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-slate-100">{t('team.title')}</h1>
        ...
        <button onClick={() => setModal({ data: null })} className="z-btn-primary flex items-center gap-2 self-start sm:self-auto">
          <PlusIcon className="w-4 h-4" /> {t('team.add')}
        </button>
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm min-w-[500px]">
            <thead className="bg-black/20">
              <tr>
                {[t('team.headers.name'), t('team.headers.email'), t('team.headers.role'), t('team.headers.status'), t('team.headers.actions')].map(h => (
            ...
            <tbody className="divide-y divide-z-border">
              ...
              {members.length === 0 && (
                <tr><td colSpan={5} className="px-6 py-10 text-center text-slate-500">{t('team.noMembers')}</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
function MemberModal({ member, onClose, onSaved }) {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      alert(err.response?.data?.detail || t('team.errorGeneric'))
      setLoading(false)
    }
  }
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-z-card border border-z-border rounded-2xl w-full max-w-md">
        <div className="flex items-center justify-between p-6 border-b border-z-border">
          <h2 className="text-lg font-bold text-slate-100">{member ? t('team.editTitle') : t('team.newTitle')}</h2>
          <button onClick={onClose}><XMarkIcon className="w-6 h-6 text-slate-500" /></button>
        </div>
        <form onSubmit={submit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-slate-300 mb-1">{t('team.fullName')}</label>
            <input value={form.full_name} onChange={e => set('full_name', e.target.value)} required className="z-input" />
          </div>
          {!member && (
            <>
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">{t('team.email')}</label>
          ...
          {member && (
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              <span className="text-sm text-slate-300">{t('team.activeUser')}</span>
            </label>
          )}
          <div className="flex justify-end gap-3 pt-2">
            <button type="button" onClick={onClose} className="z-btn-ghost">{t('team.cancel')}</button>
            <button type="submit" disabled={loading} className="z-btn-primary disabled:opacity-50">
              {loading ? t('team.saving') : t('team.save')}
            </button>
          </div>
        </form>
      </div>
