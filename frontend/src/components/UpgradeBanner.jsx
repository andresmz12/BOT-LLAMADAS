...
export default function UpgradeBanner({ demosUsed = 0, demosTotal = 10, compact = false }) {
    ...
    return (
      <div className="flex items-center gap-3 px-4 py-3 bg-amber-500/10 border border-amber-500/30 rounded-xl text-sm">
        ...
        <p className="text-amber-300 flex-1">
          <span className="font-semibold">{t('upgrade.limitedPlan')}</span> — {t('upgrade.demosUsed', { used: demosUsed, total: demosTotal })}
          {' '}{t('upgrade.requiresProOrEnterprise')}
        </p>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16 px-6 text-center">
      ...
      <h2 className="text-xl font-bold text-slate-100 mb-2">{t('upgrade.proOrEnterpriseTitle')}</h2>
      <p className="text-slate-400 text-sm max-w-md mb-2">
        {t('upgrade.currentPlanNote', { used: demosUsed, total: demosTotal })}
